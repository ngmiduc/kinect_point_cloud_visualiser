...
Rectangle space = new Rectangle(-500,-500,-500,1000,1000,1000); 
 ...
 // line(0,0,0,0,0,500); //z
  ...
  for (int x = 0; x < k1.width; x += skip) {
    for (int y = 0; y < k1.height; y += skip) {
      ...
      if ((rawDepth2 <maxDepth)&&(rawDepth2 > minDepth)){
       
        PVector v = depthToWorld(x, y, rawDepth2);
        PVector p2 = new PVector((-k1.width/2 + x)*scale, (-k1.height/2 + y)*scale, (factor*correction - v.z*factor)*scale);
        p2.x = -p2.x;
        p2.z = -p2.z;
        
        if (trans)
         stroke(c,floor(map(p2.z,-100,20,20,255)));
